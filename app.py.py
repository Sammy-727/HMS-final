...
def init_db():
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS booking_guests(
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        booking_id INTEGER,
        name TEXT,
        age INTEGER,
...
def add_booking():
    ...
    if not name or not phone or not room_id or not checkin or not checkout:
        flash("Register customer details first, then select room and dates.", "danger")
    ...
    if not room or room["status"] != "Available":
        flash("Selected room is not available. Please choose another room.", "danger")
        return redirect(url_for("manage") + "#bookings")

    existing_customer = query("SELECT * FROM customers WHERE phone=?", (phone,), one=True)
    if existing_customer:
        customer_id = existing_customer["id"]
        query(
            "UPDATE customers SET name=?, email=?, id_proof=?, address=? WHERE id=?",
            (name, email, id_proof, address, customer_id),
            commit=True
        )
    else:
        customer_id = query(
    ...
    guest_ages = request.form.getlist("guest_age[]")
    guest_relations = request.form.getlist("guest_relation[]")
    guest_ids = request.form.getlist("guest_id_proof[]")

    for i, guest_name in enumerate(guest_names):
        guest_name = guest_name.strip()
        if guest_name:
            age = guest_ages[i] if i < len(guest_ages) and guest_ages[i] else None
            relation = guest_relations[i] if i < len(guest_relations) else ""
            guest_id = guest_ids[i] if i < len(guest_ids) else ""
            query(
                "INSERT INTO booking_guests(booking_id,name,age,relation,id_proof) VALUES(?,?,?,?,?)",
                (booking_id, guest_name, age, relation, guest_id),
                commit=True
            )

    query("UPDATE rooms SET status='Occupied' WHERE id=?", (room_id,), commit=True)
    flash("Customer registered and booking created successfully.", "success")
    return redirect(url_for("manage") + "#bookings")
...
def invoice(booking_id):
    ...
    guests = query("SELECT * FROM booking_guests WHERE booking_id=? ORDER BY id", (booking_id,))

    return render_template("invoice.html", bill=bill, services=services, guests=guests)
...
@app.route("/employees/status/<int:employee_id>", methods=["POST"])
def update_employee_status(employee_id):
    ...
    if status not in ["Active", "Inactive"]:
        flash("Invalid employee status.", "danger")
        return redirect(url_for("admin") + "#employees")

    query("UPDATE employees SET status=? WHERE id=?", (status, employee_id), commit=True)
    flash("Employee activity status updated.", "success")
    return redirect(url_for("admin") + "#employees")


@app.route("/employees/drop/<int:employee_id>", methods=["POST"])
def drop_employee(employee_id):
    if session.get("role") not in ["Admin", "Manager"]:
        flash("Access denied.", "danger")
        return redirect(url_for("dashboard"))

    query("DELETE FROM employees WHERE id=?", (employee_id,), commit=True)
    flash("Employee dropped successfully.", "success")
    return redirect(url_for("admin") + "#employees")
